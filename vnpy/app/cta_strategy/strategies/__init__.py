...
def loadStrategyModule(moduleName):
    """使用importlib动态载入模块"""
    #print ('-' * 20)
    print ('import strategy file %s:' %moduleName)
...
for root, subdirs, files in os.walk(path):
    for name in files:
        ...
        if 'strategy' in name and name[-3:] == '.py':
            ...
            if basename == "strategy":
                moduleName = 'vnpy.app.cta_strategy.' + name.replace('.py', '')
            else:
                moduleName = 'vnpy.app.cta_strategy.' + basename + '.' + name.replace('.py', '')
